...
def linkedin_scraper(webpage, page_number,filename=None,writer=None,file=None):
    """this function scraps linkedIn given a webpage consisting of a job keyword and a location
    ...
    if page_number < 25:
        ...
        filename = linkedin_scraper(webpage, page_number,filename=filename, writer=writer,file=file)
    else:
        file.close()
        print('File closed')
    return file
...
if  __name__== '__main__':
    ...
    url = 'https://www.linkedin.com/jobs/search?keywords='+ ''.join(top_skills[0])+'&location='+''.join(city) + '&trk=public_jobs_jobs-search-bar_search-submit&position=1&pageNum='
    file=linkedin_scraper(url, 0, filename=filename, writer=writer,file=file)